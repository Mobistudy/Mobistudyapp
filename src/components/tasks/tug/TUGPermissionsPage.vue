<template>
  <q-page padding>
    ...
    <div class="mobitxt1 q-pa-md">{{ $q.platform.is.ios ? $t('tasks.tug.OSpermissioniOS') :
      $t('tasks.tug.OSpermissionAndroid') }}</div>
  ...
  methods: {
    ...
    async requestPermission () {
      ...
      try {
        ...
        this.$router.replace({ name: 'tug', params: { studyKey: this.studyKey, taskId: this.taskId } })
      } catch (error) {
