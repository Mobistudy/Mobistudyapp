...
export function generateTasker (studiesParts, studiesDescr, now = new Date()) {
  ...
  for (const studyPart of studiesParts) {
    if (studyPart.currentStatus === 'accepted') {
      const studyDescr = studiesDescr.find(sd => {
      ...
      for (const taskDescription of consentedTasks) {
        ...
        if (taskDescription.scheduling.alwaysOn) {
          ...
          const startOfToday = new Date(now.valueOf())
          startOfToday.setHours(0, 0, 0, 0)
          const endOfToday = new Date(now.valueOf())
          ...
          const rrule = generateRRule(taskDescription.scheduling, studyPart, studyEndDate)
          if (!rrule) continue // if the task should not start at all, go to the next task

          const instancesToEnd = rrule.between(toUTC(startOfToday), toUTC(studyEndDate))
          ...
          if (lastCompletionTS) {
            ...
            if (missed.length > 0) {
              ...
              if (taskDescription.scheduling.validitySecs) {
                ...
                const validUntil = new Date(missed.getTime() + (taskDescription.scheduling.validitySecs * 1000))
                if (now > validUntil) {
                  // if the task is not valid anymore, then it should not be scheduled
                  missed = null
                }
              }
            } else {
...
export function generateRRule (scheduling, studyPart, studyEnd) {
  ...
  if (scheduling.startEvent === 'consent') {
    ...
    // find the last time the task was performed
    if (scheduling.eventTaskId === undefined) throw new Error('Scheduling with taskExecution event must specify a taskId')
    const taskPart = studyPart.taskItemsConsent.find(t => t.taskId === scheduling.eventTaskId)
  ...
  if (scheduling.untilSecs) {
    const untilTime = new Date(startEventDate.getTime() + (1000 * scheduling.untilSecs))
...
export function scheduleNotificationsAllStudies (studiesParts, studiesDescr, now = new Date()) {
  for (const studyPart of studiesParts) {
    const acceptedTS = studyPart.acceptedTS
    const studyDescr = studiesDescr.find(sd => {
      return sd._key === studyPart.studyKey
    })
    scheduleNotificationsSingleStudy(acceptedTS, studyDescr, studyPart, now)
  }
}
 ...
 * @param {Object} studyDescr - study description
 * @param {Object} studyPart - study participation object
 * @param {Date} now - the current date
 */
export async function scheduleNotificationsSingleStudy (studyDescr, studyPart, now = new Date()) {
  ...
  for (const task of studyDescr.tasks) {
    ...
    const rrule = generateRRule(task.scheduling, studyPart, studyDescr.generalities.endDate)
    if (!rrule) continue
