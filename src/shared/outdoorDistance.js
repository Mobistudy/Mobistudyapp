...
export default {
  ...
  */
  stopTest: function () {
    ...
    let selected = this.selectPosition(this.positions[0].timestamp, 10) // for the last sample, let's try to focus on the last 10 seconds
  ...
  addPosition: function (position) {
    ...
    if (this.started) {
      ...
      if ((position.timestamp - this.selectedPositions[0].timestamp) >= (this.SELECTION_PERIOD * 1000)) {
        ...
        const selected = this.selectPosition(position.timestamp, this.SELECTION_PERIOD / 4)
        if (selected) {
          this.distance += this.crowDist(this.selectedPositions[0], selected)
          this.selectedPositions.unshift(selected)
          return true
  ...
  */
  getDistance: function () {
    if (this.started) {
      ...
      const d = this.crowDist(this.selectedPositions[0], this.positions[0])
  ...
  crowDist: function (point1, point2) {
    ...
    const lon1 = point1.coords.longitude
    const lon2 = point2.coords.longitude
    const R = 6371 // km
