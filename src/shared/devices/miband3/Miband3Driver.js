...
const Miband3 = {
  ...
  setNightMode: async function (startTime, endTime) {
    ...
    if (startTime !== null && endTime !== null) {
      const startHour = this.createByteStringFromInt(startTime.getHours())
      const startMinute = this.createByteStringFromInt(startTime.getMinutes())
      const endHour = this.createByteStringFromInt(endTime.getHours())
      const endMinute = this.createByteStringFromInt(endTime.getMinutes())
      const schedule = '01'
  ...
  setDisplayDateTime: async function () {
    const dateAndTime = '03'
    // let timeOnly = '00' // Does not work currently, for some reason.
    const command = this.messages.setup.dateTimeFormat
    const packet = this.hexStringToHexBuffer(
      command + dateAndTime
    )
    await this.sendAndVerifyConfiguration(
  ...
  setTimeFormat: async function (format) {
    let packet = ''
    const command = this.messages.setup.hourFormat
    if (format === '12h') {
      const twelveHourFormat = '00'
      packet = this.hexStringToHexBuffer(command + twelveHourFormat)
    } else {
      const twentyFourHourFormat = '01'
  ...
  setDateFormat: async function (dayFirst) {
    ...
    const command = this.messages.setup.dateFormat
  ...
  setDistanceType: async function (meters) {
    let packet = ''
    let message = ''
    const command = this.messages.setup.distance
    if (meters) {
      const metric = '00'
  ...
  setWearLocation: async function (right) {
    let packet = ''
    let message = ''
    const command = this.messages.setup.wearLocation
    if (right) {
      const right = '82'
  ...
  setupScreens: async function (screens) {
    let packet = ''
    const messageArray = this.messages.setup.changeScreens
    const command = '0a'
    let pos = 1
  ...
  parsePositionToString (pos) {
    const value = this.paddHex(parseInt(pos, 10).toString(16))
    return value
  },
  ...
  activateDisplayOnWristLift: async function (type, startTime, endTime) {
    let packet = ''
    let message = ''
    const command = this.messages.setup.displayOnLift
    if (type === 'on') {
      const on = '01'
      message = command + on
    } else if (type === 'off') {
  ...
  setGoalNotification: async function (enable) {
    let packet = ''
    let message = ''
    const command = this.messages.setup.goalNotification
    if (enable) {
      const on = '01'
      message = command + on
    } else {
  ...
  setUser: async function (
    ...
    const height1 = heightString.substring(2, 4) // Is this the right order? Little or big endian, not certain, needs to ask Dario.
    const height2 = heightString.substring(0, 2)

    const weightString = this.paddHexToBytes(
      this.createByteStringFromInt(weight * 200),
      4
    )
    const weight1 = weightString.substring(2, 4) // Same, little or big endian?
    const weight2 = weightString.substring(0, 2)

    const birthYearString = this.createByteStringFromInt(birthYear)
    const birthYear1 = birthYearString.substring(2, 4)
    const birthYear2 = birthYearString.substring(0, 2)
    const birthMonth1 = this.createByteStringFromInt(birthMonth)
    const birthDay1 = this.createByteStringFromInt(birthDay)

    const sex1 = sex ? '01' : '00' // 01 if true/female, 00 if false/male
  ...
  createByteStringFromInt (value) {
    const hexInt = parseInt(value, 10)
    const byteString = this.paddHex(hexInt.toString(16))
    return byteString
  },
   ...
   * @param {Date} startDate the date from which to start fetching data
  ...
  getTotalSamplesFromBuffer (buffer) {
    const totalSamples = (buffer[6] << 24) | (buffer[5] << 16) | (buffer[4] << 8) | (buffer[3])
    return totalSamples
  },
 ...
 */
  ...
  createDateFromHexString: function (hexString) {
    ...
    const month = parseInt(hexString.substring(4, 6), 16) - 1
    const day = parseInt(hexString.substring(6, 8), 16)
    const hour = parseInt(hexString.substring(8, 10), 16)
    const minutes = parseInt(hexString.substring(10, 12), 16)
    const seconds = 0
  ...
  sendStartDateAndActivity: async function (date, activityType) {
    ...
    const dateMessage = customDate.getDateStringPacket()
    const packet = this.hexStringToHexBuffer(
  ...
  startStepCountMonitoring: async function (dataCallback) {
    ...
    return new Promise((resolve, reject) => {
      window.ble.startNotification(
        ...
        this.stepCountCharacteristic,
        dataResponse => {
          const data = Buffer.from(dataResponse).toString('hex')
          dataCallback({
            timestamp: new Date(),
  ...
  startRawDataMonitoring: async function (ppg, acceleration, dataCallback) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (ppg && acceleration) {
        const enableMessage = this.messages.rawData.rawHRAndACC
        ...
        const enableMessage = this.messages.rawData.rawAcc
        this.enableRawData(enableMessage)
          .then(this.sendStartRawDataCommand())
          .then(
            this.pingRawData(
        ...
        const enableMessage = this.messages.rawData.rawHR
        this.enableRawData(enableMessage)
          .then(this.turnOnContinuousHRMeasuring())
          .then(this.sendStartRawDataCommand())
          .then(
  ...
  createSeveralFromSingleRawData: function (rawData, type) {
    ...
    for (let i = 0; i < rawData.length; i += 2) {
      if (type === 'HR') {
        const tot = (rawData[i + 1] << 8) | rawData[i]
        rawDataArray.push(tot)
      } else if (type === 'ACC') {
        const tot = (rawData[i + 1] << 8) | rawData[i]
        const value = this.fromTwosComplement(tot, 2)
  ...
  fromTwosComplement: function (twosComplement, numberBytes) {
    ...
    const throwString = "Two's complement out of range given " +
      numberBytes +
      ' byte(s) to represent.'
  ...
  enableRawData: async function (message) {
    const packet = this.hexStringToHexBuffer(message)
  ...
  startHRNotifications: function (callback) {
    ...
    window.ble.startNotification(
      ...
      this.hrMonitorMeasureCharacteristic,
      responseData => {
        const value = Buffer.from(responseData)
        const hrValue = '0x' + value.toString('hex')
  ...
  pingHRControlMonitor: async function () {
    this.hrPingTimer =
      this.hrPingTimer ||
      setInterval(() => {
        const packet = this.hexStringToHexBuffer(this.messages.heartRate.ping)
  ...
  getTimeStatus: async function () {
    ...
    const year = dataBuffer.readUInt16LE(0),
      mon = dataBuffer[2] - 1,
      day = dataBuffer[3],
      hrs = dataBuffer[4],
      min = dataBuffer[5],
  ...
  setTimeStatus: async function (date) {
    if (DEBUG) console.log('Setting time:', date)
    const customDate = new CustomDate(date)
    const packetContent = customDate.getDateStringPacket()
    const packet = this.hexStringToHexBuffer(packetContent)
  ...
  getHardwareInfo: async function () {
    ...
    const dataString = this.dataToASCII(new Uint8Array(data))
    return dataString
  },
  ...
  getSerialNumber: async function () {
    const data = await this.read(this.deviceInformationService, this.serialNumberCharacteristic)
    const dataString = this.dataToASCII(new Uint8Array(data))
    return dataString
  },
  ...
  dataToASCII: function (data) {
    const dataString = this.textDecoder.decode(data)
    return dataString
  },
