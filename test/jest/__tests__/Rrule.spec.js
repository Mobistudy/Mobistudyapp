...
describe('When testing RRules', () => {
  test('a daily sch is not available before start and available after start', () => {
    const startDate = new Date(2020, 1, 10, 12, 30, 0, 0)
    const rr = new RRule({
      freq: RRule.DAILY,
      interval: 1,
      dtstart: toUTC(startDate)
    })
    const before = rr.before(toUTC(new Date(2020, 1, 10, 12, 29, 0, 0)))
  ...
  test('a daily schedule starts at the beginning of the day', () => {
    const startEvent = new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 15)
    ...
    const instances = rr.between(toUTC(startOfToday), toUTC(new Date()))
    expect(instances.length).toBe(1)
    const dd = fromUTC(instances[0])
    expect(dd.getHours()).toBe(0)
  })

  test('a daily schedule with hours starts at that hour', () => {
    const startEvent = new Date(2020, 1, 10, 12, 0, 0, 0, 1)
    const rr = new RRule({
      freq: RRule.DAILY,
      interval: 1,
      dtstart: toUTC(startEvent),
    ...
    const before = new Date(2020, 1, 15, 18, 59, 59, 59, 999)
    const after = new Date(2020, 1, 15, 19, 1, 1, 1, 1)

    const instances = rr.between(toUTC(before), toUTC(after))
    expect(instances.length).toBe(1)
    const dd = fromUTC(instances[0])
  ...
  test('a daily schedule with 2 hours starts at those hour', () => {
    ...
    const after = new Date(2020, 1, 15, 0, 0, 0, 0, 1)
    const before = new Date(2020, 1, 16, 23, 59, 59, 59, 999)

    const instances = rr.between(toUTC(after), toUTC(before))
